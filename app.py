...
def edit_db():
    ...
    if password != "IzabelaChatbot2025":
        return "Acesso não autorizado", 401

    current_content = db_crú.db_load()
    return render_template('admin.html', current_db=current_content)

#http://localhost:5000/save-db?password=IzabelaChatbot2025
...
def save_db():
    ...
    new_content = request.form['db_content']
    db_crú.replace_db(new_content)

    # Recria o vetorstore
    raw_text = db_crú.db_load()
    documents = [Document(page_content=raw_text)]
    splits = text_splitter.split_documents(documents)
    db = FAISS.from_documents(splits, embeddings)
    db.save_local("db_directory")

    return redirect(url_for('edit_db', password=password))
...
def ask():
    ...
    if use_faq:
        ...
        db = FAISS.load_local(
            r"C:\Users\lucas\PycharmProjects\i10Chatbot\db_directory",
            embeddings,
